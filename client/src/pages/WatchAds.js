...
import { getAdsStatus, startAd, completeAd } from '../services/api';
...
function WatchAds({ user, updatePoints }) {
  ...
  const completeAdWatch = async () => {
    try {
      ...
      const res = await completeAd('rewarded_ad', watchDuration, sessionToken);
      updatePoints(res.data.newBalance);
      setStatus(prev => ({
        ...prev,
        adsWatched: prev.adsWatched + 1,
        adsRemaining: res.data.adsRemaining ?? Math.max(0, prev.adsRemaining - 1),
      }));
    ...
    <div style={{ padding: '0 0 24px', position: 'relative', zIndex: 1 }}>
      <div style={{ padding: '24px 20px 0', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px', marginTop: '4px' }}>
          {t('adsDesc')} · +{status.pointsPerAd} {t('points')}
        </p>
      </div>
      ...
      <div style={{ margin: '20px 16px 0' }}>
        <div className="stats-grid" style={{ padding: 0 }}>
          ...
          <div className="stat-card">
            ...
            <div className="stat-label">{t('todayRemaining')}</div>
          </div>
        </div>
        ...
        <div style={{ marginTop: '12px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '16px' }}>
          ...
          <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)', textAlign: 'center', marginTop: '6px' }}>
            {((status.adsWatched || 0) * status.pointsPerAd).toLocaleString()} {t('points')}
          </div>
        </div>
      </div>
      ...
      <div style={{ margin: '16px' }}>
        {watching ? (
          <div className="card card-blue" style={{ textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '15px', fontWeight: '700', color: '#60a5fa', marginBottom: '20px' }}>
              ⏳ {timeLeft}s
            </div>
            ...
            <button className="glow-btn pink" onClick={cancelWatch} style={{ fontSize: '13px', padding: '10px 20px' }}>
              ❌
            </button>
          </div>
        ) : (
          <div className="card" style={{ textAlign: 'center' }}>
            {status.adsRemaining > 0 ? (
              <>
                ...
                <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.4)', marginBottom: '20px' }}>{t('points')}</div>
                <button className="glow-btn green" onClick={startWatching}>
                  {t('watchBtn')} (+{status.pointsPerAd} {t('points')})
                </button>
              </>
            ) : (
              <>
