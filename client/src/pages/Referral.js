...
function Referral({ user }) {
  ...
  const fetchData = async () => {
    try {
      ...
      setInfo(infoRes.data); setFriends(listRes.data.referrals || []);
  ...
  const handleCopy = () => {
    ...
    const done = () => { setCopied(true); toast.success(`${t('copied')}`); setTimeout(() => setCopied(false), 2500); };
    if (navigator.clipboard) {
      navigator.clipboard.writeText(info.referralLink).then(done).catch(() => { const el = document.createElement('textarea'); el.value = info.referralLink; document.body.appendChild(el); el.select(); document.execCommand('copy'); document.body.removeChild(el); done(); });
    } else {
      const el = document.createElement('textarea'); el.value = info.referralLink; document.body.appendChild(el); el.select(); document.execCommand('copy'); document.body.removeChild(el); done();
    }
  };
  ...
  const handleShare = () => {
    ...
    if (window.Telegram?.WebApp?.openTelegramLink) {
      window.Telegram.WebApp.openTelegramLink(`https://t.me/share/url?url=${encodeURIComponent(info.referralLink)}&text=${encodeURIComponent('🎰 Join & Earn!')}`);
    } else if (navigator.share) {
      navigator.share({ title: 'RewardSpin', url: info.referralLink });
    } else { handleCopy(); }
  };
  ...
  return (
    <div style={{ padding: '0 0 24px', position: 'relative', zIndex: 1 }}>
      <div style={{ padding: '24px 20px 0', textAlign: 'center' }}>
        ...
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '13px', marginTop: '6px' }}>{t('referralDesc')}</p>
      </div>
      ...
      <div style={{ margin: '20px 16px 0' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
          <div style={{ background: 'rgba(139,92,246,0.1)', border: '1px solid rgba(139,92,246,0.3)', borderRadius: '16px', padding: '16px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', marginTop: '4px' }}>{t('youGet')}</div>
          </div>
          <div style={{ background: 'rgba(16,185,129,0.1)', border: '1px solid rgba(16,185,129,0.3)', borderRadius: '16px', padding: '16px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.5)', marginTop: '4px' }}>{t('friendGet')}</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ margin: '16px' }}>
        <div className="card card-purple">
          <div className="section-header">
            ...
            <div>
              <div className="section-title">{t('yourLink')}</div>
              <div className="section-subtitle">{t('shareLink')}</div>
            </div>
          </div>
          <div className="referral-link-box">{info?.referralLink || '...'}</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
      ...
      {friends.length > 0 ? (
        <div style={{ margin: '0 16px' }}>
          <div className="card">
            <div className="section-header" style={{ marginBottom: '8px' }}>
              ...
              <div><div className="section-title">{t('friends')} ({friends.length})</div></div>
            </div>
            {friends.map((f, i) => (
              <div key={i} className="friend-item" style={{ borderBottom: i < friends.length - 1 ? '1px solid rgba(255,255,255,0.06)' : 'none' }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)', marginTop: '2px' }}>
                    {f.username ? `@${f.username}` : '—'} · {new Date(f.joinedAt).toLocaleDateString()}
                  </div>
                </div>
        ...
        <div style={{ margin: '0 16px' }}>
          <div className="card" style={{ textAlign: 'center', padding: '32px' }}>
            ...
            <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '14px' }}>{t('noFriends')}</div>
            <div style={{ color: '#8b5cf6', fontSize: '13px', marginTop: '6px' }}>{t('noFriendsSub')}</div>
          </div>
        </div>
      )}
      ...
      <div style={{ margin: '16px' }}>
        <div className="card">
          <div style={{ fontSize: '14px', fontWeight: '700', color: 'rgba(255,255,255,0.7)', marginBottom: '12px' }}>{t('howItWorks')}</div>
          {[[`1️⃣`, t('step1')],[`2️⃣`, t('step2')],[`3️⃣`, t('step3')],[`4️⃣`, t('step4')]].map(([num, text], i) => (
